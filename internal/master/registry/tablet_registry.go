...
// GetEndpoint returns the RPC endpoint for the tablet server that hosts
// the given tablet. It selects the first available replica (leader).
func (r *TabletRPCRegistry) GetEndpoint(tabletID string) (string, error) {
	...
	// Select the first available replica (simple leader selection)
	// In production, this should use raft leader information
	for tsUUID := range view.Replicas {
		desc, ok := r.tsManager.Get(tsUUID)
		if !ok {
			continue
		}
		// Build RPC URL from the tablet server's RPC address
