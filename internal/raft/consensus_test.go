...
func TestHandleAppendEntriesTermRules(t *testing.T) {
	...
	resp, err = c.HandleAppendEntries(context.Background(), raft.AppendEntriesRequest{
		Term:     6,
		LeaderID: "leader-c",
		Entries: []wal.Entry{{OpID: types.OpID{Term: 6, Index: 1}, Payload: []byte("z")}},
...
func TestHandleAppendEntriesRejectsPrevLogMismatch(t *testing.T) {
	c, l := newConsensusForTest(t, "n-prev")
	defer l.Close()
	defer c.Stop()
	...
	resp, err := c.HandleAppendEntries(context.Background(), raft.AppendEntriesRequest{
		Term:        2,
		LeaderID:    "leader-x",
		PrevLogOpID: types.OpID{Term: 2, Index: 99},
		Entries: []wal.Entry{{OpID: types.OpID{Term: 2, Index: 100}, Payload: []byte("new")}},
	})
	if err != nil {
		t.Fatalf("append entries: %v", err)
	}
	if resp.Success {
		t.Fatalf("expected append reject for missing prev log op")
	}
}

func TestHandleAppendEntriesAcceptsWhenPrevLogExists(t *testing.T) {
	c, l := newConsensusForTest(t, "n-prev-ok")
	defer l.Close()
	defer c.Stop()

	// seed existing op
	if err := l.AppendSync(context.Background(), []wal.Entry{{OpID: types.OpID{Term: 2, Index: 10}, Payload: []byte("seed")}}); err != nil {
	...
	resp, err := c.HandleAppendEntries(context.Background(), raft.AppendEntriesRequest{
		Term:        2,
		LeaderID:    "leader-x",
		PrevLogOpID: types.OpID{Term: 2, Index: 10},
		Entries: []wal.Entry{{OpID: types.OpID{Term: 2, Index: 11}, Payload: []byte("next")}},
		LeaderCommit: types.OpID{Term: 2, Index: 11},
	})
	if err != nil {
		t.Fatalf("append entries: %v", err)
	}
	if !resp.Success {
		t.Fatalf("expected append success when prev log exists")
	}
}
...
func TestElectionTimerPromotesCandidateDeterministicTick(t *testing.T) {
	dir := t.TempDir()
	l, err := wal.NewLog(wal.Config{Dir: dir})
	if err != nil {
		t.Fatalf("new wal: %v", err)
